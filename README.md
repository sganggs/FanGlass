...
4. 首次启动时，FanGlass 会弹出一次授权提示：
   ...
   > **FanGlass 需要一次管理员授权**
   > 风扇转速的写入需要 root 权限。FanGlass 会安装一个后台助手来完成写入，只需授权一次；日常使用不会再要求密码（仅在升级或卸载助手时会再询问一次）。温度读取不需要任何权限。
   >
   > 助手会作为后台服务常驻（开机自启，退出 FanGlass 后仍在运行），可随时在「设置 → 特权助手」中卸载。

   点**安装助手**并输入登录密码。装好后窗口右上角的状态标记会变成「助手已连接」，就可以开始调风扇了。

如果当时点了**稍后**也没关系 —— 之后任何时候选择「固定转速」、曲线预设或菜单栏的快捷模式，FanGlass 都会就地再问一次，不需要你去设置里找。菜单栏面板有「安装助手」按钮，风扇页在助手未安装或版本过旧时会常驻一条提示条，窗口右上角的状态标记本身也可以点（「助手未安装 · 安装」／「助手版本过旧 · 更新」）。
