...
4. 首次启动时，FanGlass 会弹出一次授权提示：
   ...
   > **FanGlass 需要一次管理员授权**
   > 写入风扇转速需要 root 权限，因此需要一次管理员授权，安装负责写入的后台助手。
   >
   > 升级或卸载助手时会再询问一次；助手常驻后台，可随时在「设置 → 特权助手」中卸载。

   点**安装助手**并输入登录密码。装好后窗口右上角的状态标记会变成「助手已连接」，就可以开始调风扇了。

如果当时点了**稍后**也没关系 —— 之后任何时候选择「固定转速」、曲线预设或菜单栏的快捷模式，FanGlass 都会就地再问一次，不需要你去设置里找。菜单栏面板有「安装…」按钮，风扇页在助手未安装或版本过旧时会常驻一条提示条，窗口右上角的状态标记本身也可以点（「助手未安装 · 安装」／「助手版本过旧 · 更新」）。
...
- **设置**：界面语言（跟随系统 / 简体中文 / English）、采样间隔（0.5–3 秒）、登录时启动、退出时是否立即恢复自动、曲线温度源、转速迟滞、过热提醒阈值，以及特权助手的安装 / 重新安装 / 卸载。
...
FanGlass.app（SwiftUI 菜单栏程序，普通用户权限）
   ...
   │  JSON-lines over /var/run/fanglass.sock（每 5 秒一次心跳指令，
   │  外加约 10 秒一次的状态查询；查询不会刷新看门狗）
   ▼
...
- **App 失联 20 秒** → 看门狗把所有风扇交还系统自动控制（睡眠唤醒造成的时钟跳变不算失联；`kill -9` 已实测可恢复）。只有真正的控制指令（`hold` / `auto`）会刷新这个计时，`ping` 与 `status` 是纯查询 —— 否则一个已经没人管的强制转速，会被 App 例行的状态查询一直续命。
- **启动时认领旧的强制转速** → App 起来后先问助手「你现在压着哪几个风扇」：这一版会继续驱动的风扇由它接管，之后能正常交还；不会驱动的（本机型不支持风扇控制，或索引不在当前风扇列表里）当场交还系统。上一个进程被强杀后留下的转速因此不会无人认领。
...
**为什么一定要输一次密码？**
macOS 只允许 root 进程写 SMC 的风扇寄存器。任何能控制 Mac 风扇的软件都必须跨过这条线，绕不过去。FanGlass 在安装助手时问一次，日常使用不会再问；只有升级助手（协议版本变了）或卸载助手时会再授权一次。
...
**状态显示「助手版本过旧」？**
launchd 会一直运行磁盘上那一份旧守护进程，新版 App 的指令它可能不认。窗口右上角的状态标记会写成「助手版本过旧 · 更新」，风扇页同时常驻一条提示条；点其中任意一个（或到设置里点「更新助手…」）重装一次即可，只需再授权一次。
...
**界面能切英文吗？**
能。界面默认跟随系统语言；想单独指定的话，在**设置 → 通用 → 语言**里选「跟随系统」、「简体中文」或「English」，重启 FanGlass 后生效（该开关写的就是系统自己那一套逐应用语言设置）。
...
- 菜单栏的快捷模式胶囊行用 `AdaptivePillRow` 布局：标签装得下就维持今天这一行等宽胶囊（中文如此），装不下就自动换行、每行各自等宽铺满整行（英文如此），不会截断成「…」。
...
- 改动助手的通信协议、或改动它的行为时，都要同步提升 `Sources/Shared/HelperProtocol.swift` 里的 `version` —— 哪怕报文格式没变。launchd 运行的是磁盘上那一份旧守护进程，「助手版本过旧」的提示是它被替换掉的唯一途径；不提版本号，改的就只是仓库里的代码。
- `scripts/install.sh` 里那段以 root 运行的脚本是在 `$(cat <<EOF … EOF)` 中拼出来的：macOS 自带的 bash 3.2 会在这段 heredoc 里继续找配对的右括号，英文注释里出现一个撇号就会让整个文件在别处报出莫名其妙的 `unexpected EOF`。该脚本用 `nc` 而不是 `python3` 去 ping 助手确认安装成功 —— 没装 Command Line Tools 的 Mac 上，`/usr/bin/python3` 是个会弹安装对话框的占位程序。
